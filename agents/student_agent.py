...
class StudentAgent(Agent):
    ...
    def calculate_dir_score(chess_board, pos, adv_pos, max_step, dir):
        ...
        our_moves = StudentAgent.find_allowed_moves(chess_board_tmp, pos, adv_pos, max_step, [])
        opponent_moves = StudentAgent.find_allowed_moves(chess_board_tmp, adv_pos, pos, max_step, [])
    ...
    def step(self, chess_board, my_pos, adv_pos, max_step):
        ...
        adv_in_reach, from_pos, from_dir = StudentAgent.adv_in_reach(chess_board, adv_pos, allowed_moves)
        ...
        for move in allowed_moves:
            score = StudentAgent.calculate_pos_score(chess_board, move, adv_pos, max_step)
        ...
        if len(allowed_dirs) > 1: # Avoid checking scores if there is only one allowed direction
            max_score = -1000
            for dir in allowed_dirs:
                score = StudentAgent.calculate_dir_score(chess_board, best_move, adv_pos, max_step, dir)
                if score > max_score:
                    max_score = score
                    best_dir = dir
