...
def total_allowed_moves(chess_board, my_pos, adv_pos, max_step, allowed_dirs):
    """
    This method use recursion to examine all the possible moves that student agent can go and returns the value
    """
    if(max_step == 0):
        return 0
    ...
    if(len(allowed_dirs) == 0):
        ...
        return 1 + total_allowed_moves(chess_board, moves[allowed_dirs[0]], adv_pos, max_step-1, allowed_dirs)
    elif(len(allowed_dirs) == 2):
        return 1 + total_allowed_moves(chess_board, moves[allowed_dirs[0]], adv_pos, max_step-1, allowed_dirs) + total_allowed_moves(chess_board, moves[allowed_dirs[1]], adv_pos, max_step-1, allowed_dirs)
    elif(len(allowed_dirs) == 3):
        return 1 + total_allowed_moves(chess_board, moves[allowed_dirs[0]], adv_pos, max_step-1, allowed_dirs) + total_allowed_moves(chess_board, moves[allowed_dirs[1]], adv_pos, max_step-1, allowed_dirs) + total_allowed_moves(chess_board, moves[allowed_dirs[2]], adv_pos, max_step-1, allowed_dirs)
...
def distance_from_nearest_corner(chess_board, my_pos, adv_pos):
    """
    This method calculates the distance from the closest corner for both the player (student agent) and opponent (random agent) and returns a tuple
    """
    ...
    if(x_A < mid):
        if(y_A < mid):
            # top left
            distance_A = manhattan_distance(top_left_corner, my_pos)
        else:
            # top right
            distance_A = manhattan_distance(top_right_corner, my_pos)
    else:
        if(y_A < mid):
            # bottom left
            distance_A = manhattan_distance(bottom_left_corner, my_pos)
        else:
            # bottom right
            distance_A = manhattan_distance(bottom_right_corner, my_pos)

    # random agent
    if(x_B < mid):
        if(y_B < mid):
            # top left
            distance_B = manhattan_distance(top_left_corner, adv_pos)
        else:
            # top right
            distance_B = manhattan_distance(top_right_corner, adv_pos)
    else:
        if(y_B < mid):
            # bottom left
            distance_B = manhattan_distance(bottom_left_corner, adv_pos)
        else:
            # bottom right
            distance_B = manhattan_distance(bottom_right_corner, adv_pos)
...
def calculate_score(chess_board, my_pos, adv_pos, max_step):
    ...
    5. Distance from the nearest corner (random agent)
    ...
    """
    _, heuristic_1 = number_of_allowed_dirs(chess_board, my_pos, adv_pos)
    print(f"Number of allowed dirs for student agent: {heuristic_1}")

    heuristic_2 = 0
    for i in range(0, max_step+1):
        heuristic_2 += total_allowed_moves(chess_board, my_pos, adv_pos, i, [])
    print(f"Total number of allowed moves for student agent: {heuristic_2}")

    heuristic_3 = manhattan_distance(my_pos, adv_pos)
    print(f"Manhattan distance between A and B: {heuristic_3}")

    heuristic_4, heuristic_5 = distance_from_nearest_corner(chess_board, my_pos, adv_pos)
    ...
    normalized_heuristic_1 = min_max_normalize(heuristic_1, 0, 4)
    normalized_heuristic_2 = min_max_normalize(heuristic_2, 0, chess_board.shape[0] ** 2)
